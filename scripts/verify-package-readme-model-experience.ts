...
const SENTENCE_MODEL_EXPERIENCE: Readonly<Record<string, SentenceContract>> = {
  ...
  'packages/identity/principal': { kind: 'none', reason: 'The identity seam decides which credential and which records a request reaches; the model sees neither decision.' },
  'packages/litellm/litellm-client': { kind: 'none', reason: 'Management-API reads only; no request this package makes reaches a model.' },
  'packages/litellm/litellm-auth': { kind: 'none', reason: 'Sign-in decides which credential a later model request carries; it registers no prompt, schema, or result text.' },
